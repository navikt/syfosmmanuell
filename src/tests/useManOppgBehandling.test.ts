...
//import { received } from '../mock/data/receivedOppg';
import { ManuellOppgave } from '../types/ManuellOppgaveTypes';
import { RuleNames } from '../types/ValidationresultTypes';

describe('useManOppgBehandling', () => {
    it('Error skal initialiseres med null', () => {
        const { result } = renderHook(() => useManOppgBehandling());
        expect(result.current.error).toBeNull();
    });

    it('Skal sette manuelle oppgaver, aktuellManoppgave og aktuellArsak ved én manuell oppgave som har én arsak', () => {
        const { result } = renderHook(() => useManOppgBehandling());
        expect(result.current.manOppgaver).toBeNull();
        expect(result.current.aktuellManOppgave).toBeNull();
        expect(result.current.aktuellArsak).toBeNull();
        act(() => {
            result.current.setManOppgaver([new ManuellOppgave(manuellOppgave)]);
        });
        expect(result.current.manOppgaver[0]).toBeInstanceOf(ManuellOppgave);
        expect(result.current.aktuellManOppgave).toBeInstanceOf(ManuellOppgave);
        expect(result.current.aktuellArsak).toEqual(RuleNames['BEHANDLER_KI_FT_MT_BENYTTER_ANNEN_DIAGNOSEKODE_ENN_L']);
        expect(
            result.current.aktuellManOppgave.validationResult.behandlet.get(
                RuleNames['BEHANDLER_KI_FT_MT_BENYTTER_ANNEN_DIAGNOSEKODE_ENN_L'],
            ),
        ).toBeNull();
    });

    it('Skal ikke sette aktuellArsak ved flere enn én arsak', () => {
        const { result } = renderHook(() => useManOppgBehandling());
        act(() => {
            result.current.setManOppgaver([new ManuellOppgave(manuellOppgaveToArsaker)]);
        });
        act(() => {
            result.current.oppdaterVurdering(true);
        });
        expect(result.current.aktuellArsak).toBeNull();
    });

    it.skip('Skal oppdatere vurdering for aktuellarsak når aktuellManOppgave har én arsak', () => {
        const { result } = renderHook(() => useManOppgBehandling());
        act(() => {
            result.current.setManOppgaver([new ManuellOppgave(manuellOppgave)]);
        });
        expect(result.current.manOppgaver.length).toEqual(1);
        expect(result.current.aktuellManOppgave).toBeInstanceOf(ManuellOppgave);
        ...
        expect(
            result.current.aktuellManOppgave.validationResult.behandlet.get(
                result.current.aktuellManOppgave.validationResult.ruleHits[0].ruleName,
            ),
        ).toBeNull();
        ...
        expect(result.current.aktuellManOppgave.sendInnValidering).toBeTruthy();
        expect(result.current.aktuellArsak).toBeNull();
        expect(
            result.current.aktuellManOppgave.validationResult.behandlet.get(
                result.current.aktuellManOppgave.validationResult.ruleHits[0].ruleName,
    ...
    it('Skal oppdatere vurdering for aktuellarsak når aktuellManOppgave har flere enn én arsak', () => {
        const { result } = renderHook(() => useManOppgBehandling());
        act(() => {
            result.current.setManOppgaver([new ManuellOppgave(manuellOppgaveToArsaker)]);
        });
        act(() => {
            result.current.setAktuellArsak(result.current.aktuellManOppgave.validationResult.ruleHits[0].ruleName);
        });
        expect(
            result.current.aktuellManOppgave.validationResult.behandlet.get(
                result.current.aktuellManOppgave.validationResult.ruleHits[0].ruleName,
    ...
    it('skal bytte aktuellManOppgave', () => {
        const { result } = renderHook(() => useManOppgBehandling());
        act(() => {
            result.current.setManOppgaver(toManuellOppgaveToArsaker.map(oppg => new ManuellOppgave(oppg)));
        });
        const aktuellOppgave = result.current.aktuellManOppgave;
        act(() => {
            result.current.byttAktuellManOppgave();
        });
        act(() => {
    ...
    it('skal oppdatere oppgaverLoest når oppgave byttes', () => {
        ...
        expect(result.current.oppgaverLoest).toBe(0);
        act(() => {
            result.current.byttAktuellManOppgave();
        });
        expect(result.current.oppgaverLoest).toBe(1);
    });
});
