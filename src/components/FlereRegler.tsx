...
const FlereRegler = ({ manOppgave, handterAvgjorelse }: FlereReglerProps) => {
    ...
    const handterVurdering = (vurdering: boolean) => {
        if (aktuellRegel) {
            setValideringsresultat(
                new ValidationResultWithStatus(valideringsresultat.setBehandlet(aktuellRegel, vurdering)),
            );
    ...
    if (!aktuellRegel) {
        return (
            <Panel tittel="En sykmelding må vurderes manuelt" border className="ekspanderbar">
                        ...
                        >
                            ...
                            {valideringsresultat.behandlet.get(regel.ruleName) === undefined ? (
                                <Knapp
                                    ...
                                    onClick={() => setAktuellRegel(regel.ruleName)}
                                >
                                    Vurder
                                </Knapp>
                            ) : (
                                <img
                                    src={
                                        valideringsresultat.behandlet.get(regel.ruleName) === false
                                            ? crossCircle
                                            : checkCircle
                                    }
